...
static bool IsExtensionAvailable(VkExtensionProperties *properties, uint32_t properties_count,
...
static void SetupVulkan(const char **extensions, uint32_t extensions_count) {
  ...
  // Create Vulkan Instance
  {
    ...
    if (IsExtensionAvailable(properties, properties_count,
...
int main(void) {
  ...
  memset((void *)&g_MainWindowData, 0, sizeof(g_MainWindowData));
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    // 1. Show the big demo window (Most of the sample code is in igShowDemoWindow()! You can browse its
