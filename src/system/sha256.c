#include "sha256.h"
#include "fs.h"

#include <stdio.h>
#include <string.h>

#define ROTR(x, n) (((x) >> (n)) | ((x) << (32 - (n))))
#define CH(x, y, z) (((x) & (y)) ^ (~(x) & (z)))
#define MAJ(x, y, z) (((x) & (y)) ^ ((x) & (z)) ^ ((y) & (z)))
#define EP0(x) (ROTR(x, 2) ^ ROTR(x, 13) ^ ROTR(x, 22))
#define EP1(x) (ROTR(x, 6) ^ ROTR(x, 11) ^ ROTR(x, 25))
#define SIG0(x) (ROTR(x, 7) ^ ROTR(x, 18) ^ ((x) >> 3))
#define SIG1(x) (ROTR(x, 17) ^ ROTR(x, 19) ^ ((x) >> 10))

static const uint32_t k[64] = {
    0x428a2f98, 0x71374491, 0xb5c0fbcf, 0xe9b5dba5, 0x3956c25b, 0x59f111f1, 0x923f82a4, 0xab1c5ed5, 0xd807aa98, 0x12835b01, 0x243185be,
    0x550c7dc3, 0x72be5d74, 0x80deb1fe, 0x9bdc06a7, 0xc19bf174, 0xe49b69c1, 0xefbe4786, 0x0fc19dc6, 0x240ca1cc, 0x2de92c6f, 0x4a7484aa,
    0x5cb0a9dc, 0x76f988da, 0x983e5152, 0xa831c66d, 0xb00327c8, 0xbf597fc7, 0xc6e00bf3, 0xd5a79147, 0x06ca6351, 0x14292967, 0x27b70a85,
    0x2e1b2138, 0x4d2c6dfc, 0x53380d13, 0x650a7354, 0x766a0abb, 0x81c2c92e, 0x92722c85, 0xa2bfe8a1, 0xa81a664b, 0xc24b8b70, 0xc76c51a3,
    0xd192e819, 0xd6990624, 0xf40e3585, 0x106aa070, 0x19a4c116, 0x1e376c08, 0x2748774c, 0x34b0bcb5, 0x391c0cb3, 0x4ed8aa4a, 0x5b9cca4f,
    0x682e6ff3, 0x748f82ee, 0x78a5636f, 0x84c87814, 0x8cc70208, 0x90befffa, 0xa4506ceb, 0xbef9a3f7, 0xc67178f2};

static void sha256_compress(sha256_ctx_t *ctx, const uint8_t block[64]) {
  uint32_t m[64];
  for (int i = 0, j = 0; i < 16; ++i, j += 4)
    m[i] = ((uint32_t)block[j] << 24) | ((uint32_t)block[j + 1] << 16) | ((uint32_t)block[j + 2] << 8) | (uint32_t)block[j + 3];
  for (int i = 16; i < 64; ++i)
    m[i] = SIG1(m[i - 2]) + m[i - 7] + SIG0(m[i - 15]) + m[i - 16];

  uint32_t a = ctx->state[0], b = ctx->state[1], c = ctx->state[2], d = ctx->state[3];
  uint32_t e = ctx->state[4], f = ctx->state[5], g = ctx->state[6], h = ctx->state[7];

  for (int i = 0; i < 64; ++i) {
    const uint32_t t1 = h + EP1(e) + CH(e, f, g) + k[i] + m[i];
    const uint32_t t2 = EP0(a) + MAJ(a, b, c);
    h = g;
    g = f;
    f = e;
    e = d + t1;
    d = c;
    c = b;
    b = a;
    a = t1 + t2;
  }

  ctx->state[0] += a;
  ctx->state[1] += b;
  ctx->state[2] += c;
  ctx->state[3] += d;
  ctx->state[4] += e;
  ctx->state[5] += f;
  ctx->state[6] += g;
  ctx->state[7] += h;
}

void sha256_init(sha256_ctx_t *ctx) {
  ctx->length = 0;
  ctx->bits = 0;
  ctx->state[0] = 0x6a09e667;
  ctx->state[1] = 0xbb67ae85;
  ctx->state[2] = 0x3c6ef372;
  ctx->state[3] = 0xa54ff53a;
  ctx->state[4] = 0x510e527f;
  ctx->state[5] = 0x9b05688c;
  ctx->state[6] = 0x1f83d9ab;
  ctx->state[7] = 0x5be0cd19;
}

void sha256_update(sha256_ctx_t *ctx, const void *data, size_t size) {
  const uint8_t *bytes = (const uint8_t *)data;
  for (size_t i = 0; i < size; ++i) {
    ctx->block[ctx->length++] = bytes[i];
    if (ctx->length == 64) {
      sha256_compress(ctx, ctx->block);
      ctx->bits += 512;
      ctx->length = 0;
    }
  }
}

void sha256_final(sha256_ctx_t *ctx, uint8_t digest[32]) {
  uint32_t i = ctx->length;
  ctx->bits += (uint64_t)ctx->length * 8;

  // Pad with a single one bit, zeroes, and the message length in bits. A tail
  // with no room for the length carries into one more block.
  ctx->block[i++] = 0x80;
  if (i > 56) {
    while (i < 64)
      ctx->block[i++] = 0x00;
    sha256_compress(ctx, ctx->block);
    i = 0;
  }
  while (i < 56)
    ctx->block[i++] = 0x00;
  for (int shift = 7; shift >= 0; --shift)
    ctx->block[i++] = (uint8_t)(ctx->bits >> (shift * 8));
  sha256_compress(ctx, ctx->block);

  for (int word = 0; word < 8; ++word)
    for (int byte = 0; byte < 4; ++byte)
      digest[word * 4 + byte] = (uint8_t)(ctx->state[word] >> (24 - byte * 8));
}

bool sha256_file_hex(const char *path, char out_hex[SHA256_HEX_SIZE]) {
  if (!path || !out_hex) return false;
  out_hex[0] = '\0';

  FILE *file = fs_open(path, "rb");
  if (!file) return false;

  sha256_ctx_t ctx;
  sha256_init(&ctx);

  unsigned char buffer[64 * 1024];
  size_t read;
  while ((read = fread(buffer, 1, sizeof(buffer), file)) > 0)
    sha256_update(&ctx, buffer, read);
  const bool ok = ferror(file) == 0;
  fclose(file);
  if (!ok) return false;

  uint8_t digest[32];
  sha256_final(&ctx, digest);
  for (int i = 0; i < 32; ++i)
    snprintf(out_hex + i * 2, 3, "%02x", digest[i]);
  return true;
}
