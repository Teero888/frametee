...
void main() {
  ...
  part_info_t foot_right = part_info_t(
		ivec2(192, 32), ivec2(64, 32),
  ...
  ivec2 eye_offsets[6] = ivec2[6](
      ivec2(64,96),  // normal eyes
      ivec2(96,96),  // angry
      ivec2(128,96), // hurt
      ivec2(160,96), // happy
      ivec2(192,96), // dead
  ...
  part_info_t eye_right = part_info_t(
      eye_offsets[frag_eye - 6], ivec2(32,32),
      vec2(0.40,0.58), vec2(0.20,0.20)
  );
  ...
  final_color = blend_pma(final_color, sample_skin(foot_shadow_left, local, frag_skin_index));
  final_color = blend_pma(final_color, sample_skin(body_shadow, local, frag_skin_index));
  final_color = blend_pma(final_color, sample_skin(foot_shadow_right, local, frag_skin_index));

  final_color = blend_pma(final_color, sample_skin(foot_left, local, frag_skin_index));
  final_color = blend_pma(final_color, sample_skin(body, local, frag_skin_index));

  final_color = blend_pma(final_color, sample_skin(eye_left, local, frag_skin_index));
  final_color = blend_pma(final_color, sample_skin(eye_right, local, frag_skin_index));

  final_color = blend_pma(final_color, sample_skin(foot_right, local, frag_skin_index));

  // final_color = blend_pma(final_color, sample_skin(hand_shadow, local, frag_skin_index));
  // final_color = blend_pma(final_color, sample_skin(hand,       local, frag_skin_index));
