...
Snippet effects are ordered, reversible transforms over authored input. The
engine owns only the ordered instances, opaque parameter bytes, undo/project
storage and derived per-stage caches. The active game owns the entire effect
catalog and every effect's meaning through `input_effect_count`,
`input_effect_desc`, `input_effect_default`, `input_effect_apply` and
`input_effect_ui`. Consequently a DDNet cleaner or target smoother never
appears in engine code, and another game may publish a completely unrelated
set of effects.

`input_effect_apply` receives an `ft_input_effect_frame` with accessors for the
input and world state around the snippet. It mutates the strided derived input
records in place; the next effect receives that output. Evaluation must be
deterministic and free of external side effects because the engine may restore
a stage's output and runtime bytes from cache. The UI callback adopts the
editor's ImGui context and edits the game's opaque parameter block. Runtime
bytes are also game-defined but are neither saved nor copied into authored
input. Playback, simulation and export consume the final cached buffer, while
disabling an effect immediately exposes its upstream stage again.
